<script lang="ts">
	...
	let loading = false;
	let message = '';
	$: item = JSON.parse(data.serializedItem);
	$: userLikes = data.userLikes;
	$: history = JSON.parse(data.serializedHistory);
	$: userHasHighBid = data.userHasHighBid;
	$: endingAt =
	...
	async function onClickLike() {
		...
		if (userLikes) {
			...
			] = await del(`/api/items/${$page.params.id}/likes`);
			userLikes = false;
		} else {
			[
				{
			...
			] = await post(`/api/items/${$page.params.id}/likes`, {});
			userLikes = true;
		}
	}
	...
	async function onSubmit() {
		...
		[, err] = await post(`/api/items/${$page.params.id}/bids`, { amount: parsed });
			...
			<div class="flex-1 flex flex-col gap-4">
				...
				{#if userHasHighBid}
					<div class="text-lg text-green-600 font-bold">
						{item.endingAt > Date.now() ? 'You have the highest bid!' : 'You won the bid!'}
					</div>
				{/if}

<script lang="ts">
	...
	import type { ItemSummary, ColumnConfig, Sort } from '$services/types';

	const dispatch = createEventDispatcher();

	export let columns: ColumnConfig[] = [];
	export let sort: Sort;
	$: items = $page.data.serializedItems.map((i: string) => JSON.parse(i)) as ItemSummary[];
	$: totalPages = Number($page.data.totalPages);
...
<table class="table table-auto p-4 w-full shadow rounded-lg bg-gray-100">
